...
export default function Paygate() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<"deposit" | "full" | "cash">(() => {
    const method = location.state?.booking?.paymentMethod || location.state?.payload?.paymentMethod;
    return method === "full" || method === "cash" ? method : "deposit";
  });
  ...
  const amountToPay = isBalancePayment
    ...
    : paymentMethod === "deposit" ? (existingBooking ? Math.round(Number(booking.total) * 0.3) : deposit) : (existingBooking ? Number(booking.total) : total);
  const paymentKind = isBalancePayment ? "balance" : paymentMethod === "deposit" ? "deposit" : "full";
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      if (!isBalancePayment && res.data?.id && res.data.paymentMethod !== paymentMethod) {
        await api.patch(`/bookings/${res.data.id}`, { paymentMethod });
      }

      if (!isBalancePayment && paymentMethod === "cash") {
        toast.success("Đã chọn thanh toán tại sân.");
        navigate("/my-bookings", {
          state: {
            successId: res.data.id,
            paymentMethod: "cash",
            payload: { ...booking, paymentMethod: "cash" },
      ...
      navigate("/my-bookings", {
        state: {
          successId: res.data.id,
          paymentMethod: isBalancePayment ? "balance" : paymentMethod,
          payload: booking,
          isAutoTransfer: false,
        },
      });
      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="bg-zinc-900 rounded-3xl border border-white/10 overflow-hidden shadow-2xl relative">
          ...
          <div className="p-8 text-center bg-gradient-to-b from-yellow-500/15 via-zinc-900 to-zinc-900 border-b border-white/5 relative">
            ...
            <div className="mt-6 bg-black/60 border border-white/10 p-4 rounded-2xl inline-flex flex-col items-center">
              <span className="text-xs uppercase font-bold text-gray-400 tracking-wider mb-1">
                {isBalancePayment ? "Thanh toán phần còn lại (70%)" : paymentMethod === "deposit" ? "Số tiền cọc giữ chỗ (30%)" : paymentMethod === "cash" ? "Thanh toán tại sân" : "Tổng tiền thanh toán 100%"}
              </span>
          ...
          <div className="p-6 md:p-8">
            {!isBalancePayment && (
              <div className="mb-8">
                <h3 className="mb-3 text-sm font-extrabold uppercase tracking-wide text-gray-300">Phương thức thanh toán</h3>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  {[
                    { id: "deposit", title: "Cọc 30%", desc: "Giữ sân trước" },
                    { id: "full", title: "Thanh toán 100%", desc: "Thanh toán toàn bộ" },
                    { id: "cash", title: "Tại sân", desc: "Trả tiền khi đến" },
                  ].map((method) => (
                    <button
                      key={method.id}
                      type="button"
                      onClick={() => setPaymentMethod(method.id as "deposit" | "full" | "cash")}
                      className={`rounded-xl border p-3 text-left transition-all ${paymentMethod === method.id ? "border-yellow-500 bg-yellow-500/15 text-white ring-1 ring-yellow-500/40" : "border-white/10 bg-black/30 text-gray-400 hover:border-yellow-500/50"}`}
                    >
                      <span className="block text-sm font-extrabold">{method.title}</span>
                      <span className="mt-1 block text-xs">{method.desc}</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {paymentMethod === "cash" && !isBalancePayment ? (
              <div className="mb-6 rounded-2xl border border-yellow-500/20 bg-yellow-500/10 p-5 text-center">
                <h3 className="mb-2 text-lg font-extrabold text-white">Thanh toán trực tiếp tại sân</h3>
                <p className="text-sm text-gray-400">Bạn không cần quét mã. Vui lòng thanh toán khi đến sân theo thời gian đã đặt.</p>
              </div>
            ) : (
            <>
            {/* Tabs */}
