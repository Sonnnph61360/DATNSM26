...
type PaymentMethod = "deposit" | "full" | "balance";
...
export default function Paygate() {
  ...
  /**
   * Không tự động mặc định về "deposit".
   *
   * Thứ tự lấy paymentMethod:
   * 1. Nếu thanh toán phần còn lại => balance
   * 2. location.state.paymentMethod
   * 3. payload.paymentMethod
   * 4. booking.paymentMethod
   */
  const selectedPaymentMethod: PaymentMethod | null = isBalancePayment
    ? "balance"
      ...
      <div className="min-h-screen bg-black text-gray-300 flex flex-col items-center justify-center py-20 px-4">
        <div className="bg-zinc-900 border border-white/10 rounded-3xl p-8 max-w-md text-center shadow-2xl">
          ...
          <p className="text-gray-400 text-sm mb-6">
            Phiên thanh toán có thể đã hết hạn hoặc dữ liệu không tồn tại.
          </p>

          <Link
            to="/"
            className="btn-primary block py-3 rounded-xl font-bold"
          >
            Quay về trang chủ
          </Link>
        </div>
  ...
  if (!isBalancePayment && !selectedPaymentMethod) {
    return (
      <div className="min-h-screen bg-black text-gray-300 flex flex-col items-center justify-center py-20 px-4">
        <div className="bg-zinc-900 border border-red-500/20 rounded-3xl p-8 max-w-md text-center shadow-2xl">
          <div className="text-5xl mb-4">⚠️</div>
          ...
          <p className="text-gray-400 text-sm mb-6">
            Không xác định được bạn đang thanh toán tiền cọc hay thanh toán
            toàn bộ đơn hàng.
          </p>
    ...
    return (
      <div className="min-h-screen bg-black text-gray-300 flex flex-col items-center justify-center py-20 px-4">
        <div className="bg-zinc-900 border border-red-500/20 rounded-3xl p-8 max-w-md text-center shadow-2xl">
          ...
          <p className="text-gray-400 text-sm mb-6">
            Dữ liệu đặt sân không hợp lệ hoặc đơn hàng chưa được tạo.
          </p>
  ...
  const bookingTotal = Number(
    existingBooking?.total ??
      balanceBooking?.total ??
      payload?.total ??
  ...
  if (isBalancePayment) {
    ...
    const balanceTotal = Number(balanceBooking?.total || 0);

    const paidAmount = Number(balanceBooking?.paidAmount || 0);

    amountToPay = Math.max(0, balanceTotal - paidAmount);
  } else if (selectedPaymentMethod === "deposit") {
    // Cọc 30%
    amountToPay = Math.round(bookingTotal * 0.3);
  } else if (selectedPaymentMethod === "full") {
    // Thanh toán 100%
    amountToPay = bookingTotal;
  }
  ...
  const remainingAmount = Math.max(0, bookingTotal - amountToPay);

  // =========================================================
  // PAYMENT KIND GỬI BACKEND
  // =========================================================

  const paymentKind: "deposit" | "full" | "balance" = isBalancePayment
    ? "balance"
  ...
  // Khi hết thời gian thì kiểm tra lại booking
  useEffect(() => {
  ...
  const handleConfirmPayment = async () => {
    ...
    // Kiểm tra paymentKind
    if (!paymentKind) {
      toast.error("Không xác định được loại thanh toán");
      return;
    }
    ...
    try {
       ...
       * BOOKING FLOW:
       *
       * Booking.tsx hiện tại đã tạo booking trước
       * rồi mới navigate sang Paygate.
       *
       * Vì vậy:
       *
       * - existingBooking => dùng booking hiện tại
       * - balanceBooking  => dùng booking hiện tại
       * - chỉ tạo booking mới nếu thực sự chưa có booking
       */
      const res =
        isBalancePayment || existingBooking
          ? { data: booking }
      ...
      if (tab === "card") {
        const vnpayRes = await api.post("/vnpay/create-url", {
          ...
          /**
           * Backend dùng paymentKind để xác định:
           *
           * deposit = 30%
      ...
      /**
       * VietQR hiện tại chưa có webhook xác thực tự động.
       *
       * Vì vậy KHÔNG được tự đánh dấu paid ở đây.
       */
      toast("Đơn đang chờ xác thực chuyển khoản.", {
        icon: "⏳",
      });
      ...
      navigate("/my-bookings", {
        state: {
          ...
          paymentMethod: isBalancePayment
            ? "balance"
          ...
          <div className="p-8 text-center bg-gradient-to-b from-yellow-500/15 via-zinc-900 to-zinc-900 border-b border-white/5 relative">
            ...
            <p className="text-gray-400 text-sm">
              ...
              <span className="text-white font-bold">
                {booking.fieldName} - {booking.court}
              </span>
            </p>
            ...
            {tab === "card" && (
              <div className="bg-black/50 border border-white/5 rounded-2xl p-6 mb-6 text-center">
                ...
                <h3 className="font-extrabold text-white text-lg mb-2">
                  Thanh toán tức thì qua VNPay
                </h3>

                <p className="text-xs text-gray-400 max-w-sm mx-auto leading-relaxed mb-4">
                  Hỗ trợ thẻ ATM nội địa, Thẻ Visa/Mastercard và ứng dụng ngân
                  hàng quét mã QR-Pay an toàn, bảo mật.
                </p>
              ...
              <div className="space-y-5 mb-6">
                ...
                <div className="bg-black border border-white/5 rounded-2xl p-5 text-sm space-y-3">
                  ...
                  <div className="flex justify-between items-center border-b border-white/5 pb-2.5">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        copyToClipboard(ACCOUNT_NO, "số tài khoản")
                      }
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        copyToClipboard(addInfo, "nội dung")
                      }
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs text-gray-400 font-medium mt-3">
                    Mở ứng dụng ngân hàng, quét đúng mã và giữ nguyên nội dung
                    chuyển khoản.
                  </p>
            ...
            {tab === "card" ? (
              ...
              <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-2xl p-4 text-center">
                ...
                <p className="text-xs text-gray-300">
                  Hệ thống đang chờ xác thực giao dịch chuyển khoản...
                </p>
              </div>
            )}
          </div>
